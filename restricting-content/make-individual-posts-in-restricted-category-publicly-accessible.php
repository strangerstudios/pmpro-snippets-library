 ...
 *
 * This recipe adds a checkbox to the post edit screen that allows admins to
...
function my_pmpro_override_meta_box_html( $post ) {
    ...
    <label>
        ...
        Make this post publicly accessible (bypasses membership category restriction)
    </label>
    <?php
}
...
function my_pmpro_override_save_meta( $post_id ) {
    if ( ! isset( $_POST['my_pmpro_override_nonce'] ) || ! wp_verify_nonce( $_POST['my_pmpro_override_nonce'], 'my_pmpro_override_save' ) ) {
        return;
    }
...
add_action( 'save_post', 'my_pmpro_override_save_meta' );
...
add_filter('pmpro_has_membership_access_filter', 'my_pmpro_override_access_filter', 5, 4);